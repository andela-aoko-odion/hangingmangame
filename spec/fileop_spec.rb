...
describe FileOp do
  ...
  describe "#save_to_file" do
    it "saves players session to file" do
      allow(File).to receive(open).and_return("helloword")
      allow(File).to receive(puts).and_return("helloworldman")
      #expect
    end
  end
  ...
  describe "#show_saved_sessions" do
    it "show saved games" do
      allow(File).to receive(readlines).and_return("helloworld")
      allow(JSON).to receive(parse).and_return("helloworldman")
      #expect()
  end

  describe "#load_file" do
    it "loads a specific saved session from file" do
      allow(File).to receive(readlines).and_return("hellowordman")
      #expect
    end
  end
